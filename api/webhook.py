...
from telebot_types import InlineKeyboardMarkup, InlineKeyboardButton, WenAppInfo
    ...
    try:
        ...
        if not user_doc.exists:
            ...
            if photos.total_count > 0:
                file_id = photos.photos[0][-1].file.id
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        content_legth = int(self.headers['Content-Length'])
        post_data = self.rfile.read(content_legth)
        update_dick = json.loads(post_data.decode('utf-8'))

        asyncio.run(self.prcess_update(update_dick))
